...
def train_model(model, train_loader, test_loader, num_epochs=100, learning_rate=0.001, task_name=""):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            print(f"Epoch [{epoch+1}/{num_epochs}], Loss: {avg_train_loss:.4f}, Acc: {test_accuracy:.2f}%")
...
if __name__ == "__main__":
    ...
    df_processed["Is_High_Intensity"] = ((df_processed["Maximum Water Height (m)"].fillna(0) >= 2) | (df_processed["Tsunami Intensity"].fillna(0) >= 3)).astype(int)
    
    input_features = ["Year", "Mo", "Dy", "Hr", "Mn", "Sec", "Tsunami Event Validity", "Tsunami Cause Code", "Earthquake Magnitude", "Vol", "Latitude", "Longitude", "Maximum Water Height (m)", "Number of Runups", "Tsunami Magnitude (Abe)", "Tsunami Magnitude (Iida)", "Tsunami Intensity"]
    ...
    X_train_sev, X_test_sev, y_train_sev, y_test_sev = train_test_split(X_scaled, y_severity, test_size=0.2, random_state=42, stratify=y_severity)
    X_train_int, X_test_int, y_train_int, y_test_int = train_test_split(X_scaled, y_intensity, test_size=0.2, random_state=42, stratify=y_intensity)
    ...
    model_severity, sev_best = train_model(model_severity, train_loader_sev, test_loader_sev, num_epochs=100, learning_rate=0.001, task_name="severity")
    print(f"Severity Model Best Accuracy: {sev_best:.2f}%")
    
    print("\nTraining Intensity Classification Model")
    model_intensity, int_best = train_model(model_intensity, train_loader_int, test_loader_int, num_epochs=100, learning_rate=0.001, task_name="intensity")
